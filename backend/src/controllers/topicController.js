...
export async function getToday(req, res, next) {
  try {
    ...
    let feedKey = user.feedKey;
    if (user.feedKeyAppliesDate && user.feedKeyAppliesDate <= date) {
      feedKey = computeFeedKey(interests);
      await db.update(users).set({ feedKey, feedKeyAppliesDate: null, updatedAt: new Date() })
...
export async function getByDate(req, res, next) {
  try {
    ...
    if (!topic) {
      [topic] = await db.select().from(dailyTopics).where(eq(dailyTopics.date, date)).limit(1);
    }
